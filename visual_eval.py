...
def split_image(image, shuffle=True):
  ps = []
  order = []
...
def visualize(org_image, split_order, pi, show=True):
  if not org_image is None:
    plt.figure()
  ...
  if not pi is None:
    ...
    for p in pi:
      xb = int(round((p[1][1] - minx) * params.patch_size))
      yb = int(round((p[1][0] - miny) * params.patch_size))
